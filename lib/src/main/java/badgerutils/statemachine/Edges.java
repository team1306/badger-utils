 ...
 *
 * <p>Edges are functions that are executed when the system transitions to another state. They may
 * do any operation other than modifying the system's state.
 *
 * <p>Similar to {@code Guards}, there are 7 types of edges, each slightly different, with very
 * different use cases.
 *
 * <ul>
 *   <li>{@link #anyToAny(StateEdge) State to state}
 *   <li>{@link #multipleStatesToMultipleStates(Set, Set, StateEdge) States to states}
 *   <li>{@link #stateToAny(Enum, StateEdge) State to any}
 *   <li>{@link #anyToState(Enum, StateEdge) Any to state}
 *   <li>{@link #anyToAny(StateEdge) Any to any}
 *   <li>{@link #stateToMultipleStates(Enum, Set, StateEdge) State to states}
 *   <li>{@link #multipleStatesToState(Set, Enum, StateEdge) States to state}
 * </ul>
 *
...
public class Edges<T extends Enum<T>> {
   ...
   * @return reference for method chaining
   */
  public Edges<T> multipleStatesToMultipleStates(
      Set<T> previousStates, Set<T> nextStates, StateEdge<T> edge) {
