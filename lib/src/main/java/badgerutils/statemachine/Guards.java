 ...
 *
 * <p>Guards are functions that are checked for true when attempting to transition to another state.
 * They may never modify a system's state
 *
 * <p>Similar to {@code Edges}, there are 7 types of guards, each slightly different, with very
 * different use cases.
 *
 * <ul>
 *   <li>{@link #anyToAny(StateGuardCondition) State to state}
 *   <li>{@link #multipleStatesToMultipleStates(Set, Set, StateGuardCondition) States to states}
 *   <li>{@link #anyToState(Enum, StateGuardCondition) State to any}
 *   <li>{@link #stateToAny(Enum, StateGuardCondition) Any to state}
 *   <li>{@link #anyToAny(StateGuardCondition) Any to any}
 *   <li>{@link #stateToMultipleStates(Enum, Set, StateGuardCondition) State to states}
 *   <li>{@link #multipleStatesToState(Set, Enum, StateGuardCondition) States to state}
 * </ul>
 *
...
public class Guards<T extends Enum<T>> {
  ...
  /**
   * Adds a 'state to state' transition for every possible pairing of states: from some states to
   * some other states
   *
   ...
   * @return reference for method chaining
   */
  public Guards<T> multipleStatesToMultipleStates(
      Set<T> previousStates, Set<T> nextStates, StateGuardCondition<T> guard) {
   ...
   * @return reference for method chaining
   */
  public Guards<T> stateToMultipleStates(
      T previousState, Set<T> nextStates, StateGuardCondition<T> guard) {
   ...
   * @return reference for method chaining
   */
  public Guards<T> multipleStatesToState(
      Set<T> previousStates, T nextState, StateGuardCondition<T> guard) {
  ...
  private void addAllPartialTransitions(
      Set<T> previousStates, Set<T> nextStates, StateGuardCondition<T> guard) {
