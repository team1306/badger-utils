 ...
 *
 * <p>It specifically enables the use of {@code StateEdge} and {@code StateGuardCondition}. <br>
 * <i>Edges</i> are functions that execute when a state changes from one state to another. An
 * example would be modifying the PID gains when transitioning to a state that requires more
 * accurate tracking. <br>
 * <i>Guards</i> are functions that control whether a state can transition to another. An example
 * would be disallowing an arm to swing through the chassis if another mechanism was in the way.
 *
 * @param <T> enum type
 */
public class StateMachine<T extends Enum<T>> {
  ...
  /**
   * Similar to {@link #StateMachine(Enum, Edges, Guards)} except that {@code stateGuards} defaults
   * to empty
   */
  ...
  /**
   * Similar to {@link #StateMachine(Enum, Edges, Guards)} except that {@code stateEdges} defaults
   * to empty
   */
  ...
  /**
   * Checks whether the current system's state can change to {@code nextState} using the {@link
   * Guards} provided.
   *
   * <p>The state is not allowed to change to itself
   *
  ...
  public boolean canChangeState(T nextState) {
    ...
    return stateGuards.getGuards(transition).stream()
        .allMatch((guard) -> guard.canChange(transition));
  }

  /**
   * Tries to change the state of the system to {@code nextState}, executing any matching {@link
   * Edges}.
   *
   * <p>Uses {@link #canChangeState(Enum)} to verify that the state can actually change.
   *
   * @param nextState the potential next state of the system
  ...
  /**
   * Changes the state of the system to {@code nextState} without checking the {@link Guards} or
   * running the {@link Edges}.
   *
   * <p>Not recommended for general use
   *
